...
public class baekjoon_17836 {
	...
	public static void bfs1() throws IOException {
		...
		while(!deque.isEmpty()) {
			...
			if(y - 1 > -1 && check1[y - 1][x] == 0 && map[y - 1][x] != 1 && time + 1 <= T) {
				check1[y - 1][x] = time + 1;
				deque.addLast(new Group(y - 1, x, time + 1, sword));
			}
			//우
			if(x + 1 < M && check1[y][x + 1] == 0 && map[y][x + 1] != 1 && time + 1 <= T) {
				check1[y][x + 1] = time + 1;
				deque.addLast(new Group(y, x + 1, time + 1, sword));
			}
			//하
			if(y + 1 < N && check1[y + 1][x] == 0 && map[y + 1][x] != 1 && time + 1 <= T) {
				check1[y + 1][x] = time + 1;
				deque.addLast(new Group(y + 1, x, time + 1, sword));
			}
			//좌
			if(x - 1 > -1 && check1[y][x - 1] == 0 && map[y][x - 1] != 1 && time + 1 <= T) {
				check1[y][x - 1] = time + 1;
				deque.addLast(new Group(y, x - 1, time + 1, sword));
			}
	...
	public static void bfs2() throws IOException {
		...
		while(!deque.isEmpty()) {
			...
			if(map[y][x] == 2) {
				sword = true;
				check2[N - 1][M - 1] = check2[y][x] + (N - 1 - y) + (M - 1 - x);
				return;
			}
			...
			if(!sword) {
				//상
				if(y - 1 > -1 && check2[y - 1][x] == 0 && map[y - 1][x] != 1
						&& time + 1 <= T) {
					check2[y - 1][x] = time + 1;
					deque.addLast(new Group(y - 1, x, time + 1, sword));
				}
				//우
				if(x + 1 < M && check2[y][x + 1] == 0 && map[y][x + 1] != 1
						&& time + 1 <= T) {
					check2[y][x + 1] = time + 1;
					deque.addLast(new Group(y, x + 1, time + 1, sword));
				}
				//하
				if(y + 1 < N && check2[y + 1][x] == 0 && map[y + 1][x] != 1
						&& time + 1 <= T) {
					check2[y + 1][x] = time + 1;
					deque.addLast(new Group(y + 1, x, time + 1, sword));
				}
				//좌
				if(x - 1 > -1 && check2[y][x - 1] == 0 && map[y][x - 1] != 1
						&& time + 1 <= T) {
					check2[y][x - 1] = time + 1;
					deque.addLast(new Group(y, x - 1, time + 1, sword));
				}
